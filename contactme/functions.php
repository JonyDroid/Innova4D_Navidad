...
function getValidationErrors()
{
    ...
    for ($i = 0; $i < count($fields); $i++) {
        $field = $fields[$i];
        $value = isset($fdata['cm_'.$i]) ? $fdata['cm_'.$i] : null;

        // Check if required
        if ($field['required'] && (($field['type'] != 'file' && empty($value)) ||
                                  ($field['type'] == 'file' && (!isset($_FILES['cm_'.$i]) || empty($_FILES['cm_'.$i]['name'][0]))))) {
        ...
        switch ($field['type']) {
            ...
            case 'file':
                ...
                if (isset($_FILES['cm_'.$i]) && !empty($_FILES['cm_'.$i]['name'][0])) {
                    $upload_errs = filesUploaded($_FILES['cm_'.$i], $field);
...
function getUserEmailFieldName()
{
    ...
    for ($i = 0; $i < count($fields); $i++) {
        if (isset($fields[$i]['is_user_email']) && $fields[$i]['is_user_email'] == true) {
            return 'cm_'.$i;
        }
    }
...
function getCustomEmailToFieldName()
{
    ...
    for ($i = 0; $i < count($fields); $i++) {
        if (isset($fields[$i]['email_to_this']) && $fields[$i]['email_to_this'] == true) {
            return 'cm_'.$i;
        }
    }
...
function getCustomEmailFromFieldName()
{
    ...
    for ($i = 0; $i < count($fields); $i++) {
        if (isset($fields[$i]['email_from_this']) && $fields[$i]['email_from_this'] == true) {
            return 'cm_'.$i;
        }
    }
