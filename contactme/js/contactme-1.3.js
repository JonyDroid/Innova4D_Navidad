...
(function($) {
	...
	$('.contactMe .form-row.file').each(function(index) {
		...
		{
			var fileName = '',
				labelHtml = '<i></i>' + $(e.target).data('displayname');
			...
			if(fileName)
				$(e.target).find('+ label').html('<i></i>' + fileName);
			else
				$(e.target).find('+ label').html(labelHtml);
		});
...
function validateField($field)
{
	...
	switch(type) {
		...
		case 'tel':
			...
			break;
		case 'file':
			if ($field.data('allowedtypes') != '' && $field[0].files.length > 0) {
				var arrTypes = $.map($field.data('allowedtypes').split(','), function(value){
